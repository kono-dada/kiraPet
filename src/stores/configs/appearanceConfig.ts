...
import type { Live2DModelInfo } from 'pages/main/live2d/live2dModelService'
...
export const useAppearanceConfigStore = defineStore(
  ...
  () => {
    ...
    const decorationType = ref<'none' | 'circle' | 'fallingStars'>('fallingStars')

    // Live2D相关设置
    const live2dBorderType = ref<'none' | 'circle'>('none')
    const live2dModelScale = ref(0.07)
    const live2dModelPositionX = ref(0.5)
    const live2dModelPositionY = ref(0.5)
