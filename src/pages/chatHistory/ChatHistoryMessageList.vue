      ...
      <div v-if="message.role === 'user'" class="message-row user-message">
        <div class="message-row__inner">
          ...
          <v-card class="message-bubble user-bubble" rounded="xl">
            <v-card-text class="pa-3">{{ defaultUserMessageFormatSpec.parse(message.content as string) }}</v-card-text>
          </v-card>
...
import { messageSpec } from '../../services/agents/character/chat';
...
// 解析 AI 消息为可渲染数组
function parseAIMessage(message: AssistantModelMessage): ParsedAIMessage[] {
  try {
    const content = typeof message.content === 'string' ? message.content : (message.content[0] as TextPart).text;
    const items = messageSpec.parse(content).sentences
    return items.map((item) => ({ chinese: item }))
  } catch (error) {
    console.log(message)
