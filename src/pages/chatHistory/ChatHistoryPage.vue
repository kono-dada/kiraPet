<template>
  <div class="chat-history-page">
    <transition name="slide-y-transition">
      <v-alert v-if="vitsWarning" type="warning" class="vits-warning-alert" variant="outlined" density="comfortable"
        icon="mdi-volume-off" closable @click:close="vitsWarning = false">
        语音播放尚未实现（TODO）
      </v-alert>
    </transition>

    <ChatHistoryToolbar :session-title="sessionTitle" :message-count="displayedMessages.length" :is-mobile="isMobile"
      :sidebar-collapsed="sidebarCollapsed" @toggle-sidebar="handleToggleSidebar">
      <v-btn class="mr-2" variant="tonal" color="primary" prepend-icon="mdi-refresh" size="small"
        :disabled="disableNewSession" @click="archiveCurrentSession">
        归档当前
      </v-btn>
      <v-btn variant="text" color="error" prepend-icon="mdi-delete-outline" size="small" :disabled="disableClear"
    ...
    <div class="chat-history-body">
      <transition name="sidebar-collapse">
        <ChatHistorySidebar v-if="!isMobile" :sessions="sidebarSessions" :active-index="activeSessionIndex"
          :collapsed="sidebarCollapsed" :is-mobile="false" :disable-clear="disableClear"
          :disable-new-session="disableNewSession" @toggle-collapse="toggleSidebarCollapse"
          @select-session="selectSession" @request-clear="clearDialog = true"
          @request-new-session="archiveCurrentSession" />
      </transition>

      <div class="chat-history-content" :class="{ 'chat-history-content--full': isMobile }">
        <div class="chat-history-panel">
          <ChatHistoryMessageList :messages="displayedMessages" :allow-delete="activeSessionIndex === -1"
            :playing-index="playingIndex" @request-delete="showDeleteDialog" @request-play="playVoice" />
        </div>
      </div>
    </div>

    <v-overlay v-if="isMobile" v-model="sidebarDrawer" class="chat-history-overlay" scrim="rgba(15, 23, 42, 0.55)"
      @click:outside="sidebarDrawer = false">
      <div class="chat-history-overlay__panel">
        <ChatHistorySidebar :sessions="sidebarSessions" :active-index="activeSessionIndex" :collapsed="false"
          :is-mobile="true" :disable-clear="disableClear" :disable-new-session="disableNewSession"
          @select-session="handleMobileSelect" @request-clear="handleMobileClear"
          @request-new-session="handleMobileNewSession" @close-mobile="sidebarDrawer = false" />
      </div>
    </v-overlay>
...
// 引入组合式 API 与 Vuetify 响应式工具
import { ref, computed, watch } from 'vue'
import { useDisplay } from 'vuetify'

// 引入拆分后的页面组件
import ChatHistorySidebar, { type SidebarSession } from './ChatHistorySidebar.vue'
import ChatHistoryToolbar from './ChatHistoryToolbar.vue'
import ChatHistoryMessageList from './ChatHistoryMessageList.vue'

// 引入会话状态管理 Store
import { HistorySession, useSessionStore } from '../../stores/session'

// 初始化会话 Store 实例
const sessionStore = useSessionStore()

// 记录当前激活的会话索引（-1 表示当前会话）
...
const sidebarSessions = computed((): SidebarSession[] => {
  const history = (sessionStore.historySessions as any[]).map((session: HistorySession, index: number) => ({
    label: session.abstract.slice(0, 10),
    time: session.closedAt,
    index,
    isCurrent: false,
    count: session.messages.length,
    preview: buildPreview(session.messages[1]?.content),
  })) as SidebarSession[]
  ...
  const currentSession: SidebarSession = {
    ...
    preview: buildPreview((sessionStore.currentSession as any[])[1]?.content),
  }
...
// 构造消息预览字符串
function buildPreview(_rawContent: any): string {
