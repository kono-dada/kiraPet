<template>
  <div class="live2d-wrapper" @mousedown="onDragStart" @click.stop="onClick">
    ...
    <div v-if="!isReady" class="avatar-loading">
...
import { initializeLive2DModels } from './live2d/live2dModelService'
import { ensureCubismCore, transformLive2DConfig } from './live2d/live2dConfigAdapter'
...
async function initLive2D() {
  ...
  try {
    ...
    try {
      ...
      console.log('Live2D 模型加载成功:', model.name)
      const expressions = live2dModel.internalModel.motionManager.expressionManager?.definitions
      if (expressions) {
        state.allExpressions = expressions.map(e => e.Name)
        console.log('表情列表:', state.getEmotionCodePrompt)
      } else {
        console.log('该模型没有表情数据')
      }
    } finally {
...
watch(() => state.currentExpression, (_newExpression) => {
  if (live2dModel) {
    try {
      live2dModel.expression(state.allExpressions[state.currentExpression])
    } catch (error) {
      console.warn('切换表情失败:', error)
    }
  }
})
