...
// 仅构造中文消息项（已移除表情与日语）
function item(zh: string) {
  return zh
}

let scenarioIndex = 0

export async function handleAvatarMultiClick(_payload: EventPayloadMap['AVATAR_MULTI_CLICK']) {
  pushLog('别点啦！我知道你很喜欢我~', getRandomColor());
  // usePetStateStore().setPetExpression(1)
  // await sleep(3000)
  // usePetStateStore().setPetExpression(0)
}
