...
mod os;
mod sbv2_manager;
mod active_window_watcher;
#[cfg(not(any(target_os = "android", target_os = "ios")))]
mod system_tray;
use commands::*;
use sbv2_manager::Sbv2Manager;
use active_window_watcher::spawn_active_window_watcher;
...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // 初始化通知窗口（隐藏且已定位），避免首次通知时再构建
            notification::init(&app.handle()).ok();

            // 启动活跃窗口监听器
            spawn_active_window_watcher(app.handle().clone());
